  ...
  try {
    const metaInfo = await Storefront.register({
      ...
      getLandingPage: async () => ({
        topRow: {
          title: "Stern Trading Platform Apps",
          items: [
            {
              id: "top-row-item-1",
              title: "All Trading Applications",
              description: "Browse all available trading applications.",
              image: {
    ...
    console.error("An error was encountered while trying to register the store provider", err);
  }
}